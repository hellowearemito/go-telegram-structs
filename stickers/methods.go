...
type UploadStickerFile struct {
	...
	PngSticker types.InputFile `json:"png_sticker"` // Png image with the sticker, must be up to 512 kilobytes in size, dimensions must not exceed 512px, and either width or height must be exactly 512px.
}
...
type CreateNewStickerSet struct {
	...
	PngSticker types.InputFile `json:"png_sticker"` // InputFile or String. Png image with the sticker, must be up to 512 kilobytes in size, dimensions must not exceed 512px, and either width or height must be exactly 512px. Pass a file_id as a String to send a file that already exists on the Telegram servers, pass an HTTP URL as a String for Telegram to get a file from the Internet, or upload a new one using multipart/form-data.
